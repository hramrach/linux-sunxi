		...
		int phandle_delta)
{
	...
	if (node->phandle != 0 && node->phandle != OF_PHANDLE_ILLEGAL) {
		pr_debug("%s: adjusting %s|%d (%s) +%d\n", __func__,
			 node->full_name, node->phandle, node->name,
			 phandle_delta);
		node->phandle += phandle_delta;
	} else
		pr_debug("%s: %s|%d (%s): invalid phandle %d\n", __func__,
			 node->full_name, node->phandle, node->name,
			 node->phandle);
		...
		struct device_node *target, int phandle_delta)
{
	...
	pr_debug("%s: %s|%d (%s), %s|%d (%s), %d\n", __func__,
		 node->full_name, node->phandle, node->name,
		 target->full_name, target->phandle, target->name,
		 phandle_delta);
	for_each_property_of_node(node, rprop) {
		...
		pr_debug(" adjusting %s|%d (%s), %s, +%d\n",
			 target->full_name, target->phandle, target->name,
			 sprop->name, phandle_delta);
		for (i = 0; i < count; i++) {
