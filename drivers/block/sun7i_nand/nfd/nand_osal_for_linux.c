...
static void NAND_DMASingleUnmap(__u32 rw, __u32 buff_addr, __u32 len);
...
void NAND_DMASingleUnmap(__u32 rw, __u32 buff_addr, __u32 len)
{
...
{
	int	cnt, i = 0;
	user_gpio_set_t * list = NULL;
	
	/* 获取gpio list */
	cnt = script_parser_mainkey_get_gpio_count("nand_para");
	if(0 == cnt) {
		printk("get nand_para gpio list failed\n");
		return;
	}
	list = kzalloc(sizeof(user_gpio_set_t)*cnt, GFP_KERNEL);
	if (!list)
		return;
	if (script_parser_mainkey_get_gpio_cfg("nand_para", list, cnt) != SCRIPT_PARSER_OK) {
		printk("nand_para gpio list parse failed\n");
		goto end;
	}
